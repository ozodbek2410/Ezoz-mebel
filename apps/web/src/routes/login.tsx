...
export function LoginPage() {
  ...
  function validate() {
    ...
    if (!username.trim()) {
      newErrors.username = _t("Login kiritilmadi");
    } else if (username.trim().length < 3) {
      newErrors.username = _t("Login kamida 3 belgi bo'lishi kerak");
    }
    if (!password) {
      newErrors.password = _t("Parol kiritilmadi");
    } else if (password.length < 4) {
      newErrors.password = _t("Parol kamida 4 belgi bo'lishi kerak");
    }
    setErrors(newErrors);
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      ...
      toast.success(`${getT()("Xush kelibsiz")}, ${result.fullName}!`);
      navigate({ to: "/dashboard" });
    } catch (err) {
      const msg = err instanceof Error ? err.message : getT()("Login yoki parol noto'g'ri");
      setLoginError(msg);
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-blue-200/60 text-sm mt-1">{t("Savdo Boshqaruv Tizimi")}</p>
        </div>

        {/* Login Form */}
        <form onSubmit={handleLogin} className="bg-white/10 backdrop-blur-xl rounded-2xl p-5 sm:p-6 border border-white/10">
          <h2 className="text-lg font-semibold text-white mb-5">{t("Tizimga kirish")}</h2>
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-sm text-blue-200/60 mb-1.5">{t("Login")}</label>
            ...
            <div>
              <label className="block text-sm text-blue-200/60 mb-1.5">{t("Parol")}</label>
              <div className="relative">
          ...
          >
            {loading ? t("Kirish...") : t("Tizimga kirish")}
          </button>
        </form>
      </div>
    </div>
